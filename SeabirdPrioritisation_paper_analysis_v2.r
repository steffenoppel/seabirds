...
### finalised on 2 Feb 2018 after including policy comments and Jonathan Green's suggestions
...
TABLE2add<- ORIG %>%
  	group_by(Family, scientific_name, breed_stage) %>%
  	summarise(MMAm=round(median(MMA)/1000,ifelse(median(MMA)<1000,2,0)), MMAl=round(min(MMA)/1000,ifelse(median(MMA)<1000,2,0)), MMAu=round(max(MMA)/1000,ifelse(median(MMA)<1000,2,0))) %>%
...
#fwrite(TABLES1,"S:\\ConSci\\DptShare\\SteffenOppel\\MANUSCRIPTS\\in_prep\\SeabirdPrioritisation\\TableS2.csv")
...
#fwrite(REPRES,"S:\\ConSci\\DptShare\\SteffenOppel\\MANUSCRIPTS\\in_prep\\SeabirdPrioritisation\\TableS3.csv")
...
### FOR ORIGINAL DATA ###
m0orig<-lmer(log(MMA)~chick+mass+COL_SIZE+n_individuals+SamplingRate+(1|colony_name)+(1|scientific_name), data=ORIG[ORIG$IBA20>0,], weights=ORIG$weight[ORIG$IBA20>0])
m1orig<-lmer(log(MMA)~chick+mass+COL_SIZE+n_individuals+Family+SamplingRate+(1|colony_name)+(1|scientific_name), data=ORIG[ORIG$IBA20>0,], weights=ORIG$weight[ORIG$IBA20>0])
...
#jpeg("S:\\ConSci\\DptShare\\SteffenOppel\\MANUSCRIPTS\\in_prep\\SeabirdPrioritisation\\Fig4.jpg", height=520, width=680, quality=100)
...
rmarkdown::render('S:\\ConSci\\DptShare\\SteffenOppel\\MANUSCRIPTS\\in_prep\\SeabirdPrioritisation\\Results_SeabirdPrioritisation_v7.Rmd',
                  output_file = "Results_v7.docx",
